...
- **Batch 5 🔵 — PITCH/VELOCITY CONDITIONING** (see below). **Model+infra side ✅ BUILT (2026-06-12, `docs/batch5-refactor.md`)**: tensor-in differentiable forward (matcher interface), f0 from note+Transpose through the physics, note/vel encoder tokens, vectorized physics, cache v2, stable hash split, pitch guard. Remaining: freq-map verification sweeps (Coarse/Fine/Fixed/Quantize laws → prior), then the full training run on the pitch-diverse dataset. Gate: pitch-conditioned renderer must match single-pitch quality, then generalize.

**Phase B — One-shot matcher (🔵 BUILT; DETERMINISTIC as of 2026-06-17, `docs/matcher-design.md`).** Pivoted from diffusion to a **deterministic direct predictor** after the diffusion version railed params to silent/insane values (gaming the renderer's peak-norm slack) and its x0-MSE proved vestigial. Current design: AudioEncoder → Algorithm classifier (the one discrete param) + a ParamHead that emits the other 194 params in one forward pass, **affine-mapped into the per-param dataset manifold box** so predictions can only land inside the audible training distribution (Volume can't rail to 0, Transpose can't exceed ±12, forced params auto-pinned). Trained with the magnitude-weighted spectral loss through the frozen renderer (driver) + param-anchor MSE + algorithm CE; true algo teacher-forced into the render; EMA. Inference predicts one preset per top-k algorithm, ranks via the neural renderer (CPU, no real render), writes op_0000=best to the predict manifest. This is the shippable form (matcher.onnx + optional renderer.onnx ranker, CPU, in-extension). CMA-ES against the REAL Operator is reclassified as a dev-only optional polish (the extension can't render instruments). Original diffusion design retained as history below.

*(Original Phase B plan — diffusion matcher — kept for the reasoning trail:)* One-shot diffusion matcher (BUILT 2026-06-15, then superseded). Renderer gate passed (+50.3% across pitch), so the renderer is frozen and now provides the audio loss. Built in `src/doppelganger/matcher/`: AudioEncoder (log-mag → embedding) → **separate Algorithm classifier** (the decided discrete-param strategy — softmax+CE, kept out of the diffusion) + FiLM-conditioned flat-vector **x0-diffusion denoiser** (MLP, NOT 2-D U-Net) over the other 194 normalized params, conditioned on [audio embed, algo embed, note, velocity]. Trained with the **magnitude-weighted spectral loss backpropped through the frozen renderer** (driver) + diffusion x0-MSE (nudge) + algo CE; true algorithm teacher-forced into the render so a misclassification can't poison the continuous matcher. Inference (`match.py`) DDIM-samples a candidate POPULATION, renders all through the frozen renderer, ranks by spectral loss, writes the ranked top-K to the predict manifest (op_0000 = best) → "Apply Predicted Batch" hears them in the real Operator. Eval gates on val match audio loss vs a predict-the-mean-params baseline (gain% > 0 = real matching) + Algorithm top-1 acc. Smoke-tested end-to-end (gradient-to-params through the frozen renderer confirmed). NEXT: train on the box vs the real renderer.pt.

**Phase C — Sim-to-real polish (🟡 DEV-ONLY, optional).** Short in-the-loop **CMA-ES against the real Operator** (`search.py`, population 64 = one export) to refine a preset; optional online renderer fine-tuning on fresh real renders. NOTE: reclassified as a dev/power-user tool — it needs real-Operator renders (export automation), so it CANNOT ship in the extension (the product is direct `audio→params` on CPU).

**Matcher inference robustness + chord data (🔵 in progress, 2026-06-17, `docs/matcher-design.md`).** Validated the matcher works in-domain (~57% of the achievable match gap on held-out renders). Hardened `match.py` for arbitrary inputs: auto pitch detection + ±octave sweep + short-clip tiling (a 0.28s chord stab went from dial-up noise → sensible timbre approximation, −32% renderer loss). NEXT: **chord training data** — Operator is monophonic so it can't reproduce a chord, but the product goal is to emulate the *instrument's timbre*. Plan: extend the collector to render presets playing multi-note CHORD clips, pair `(chord audio → that preset)`, retrain the matcher on these + the single-note data; the param-anchor teaches the chord-audio→preset map (renderer stays single-note, no change). Inference-side YIN/pitch detection + the extension (Tool 3) consume this.
...
- **Model (Batch 5) ✅ BUILT (2026-06-12):** `f0` (and velocity) threaded through `DiffOperator` (per-sample tensor; hardcoded 261.63 gone), `RendererConfig.condition_pitch`, the dataset loader (cache v2 carries note/velocity), and the conditioning encoder (two extra tokens). The previously-ignored `Transpose` now shifts f0. Inference-side f0 detection (YIN/autocorrelation) still to build with Tool 3. Verification of the remaining frequency semantics (Coarse/Fine/Fixed/Quantize) = the `synth.freq_map` sweeps + the "Collect Freq Maps" extension command.
...
**Efficiency (built):** bf16 autocast + TF32 + optional gradient checkpointing (`--no-grad-ckpt`/`--amp`), LR warmup + grad clipping (Transformer stability), eval-every-N + cached STFT windows. **Data cache v2:** `python -m doppelganger.synth.precompute` packs the dataset into `dataset/operator/cache/` (fp16 memmap `audio.npy` + normalized `params.npy` matrix + `notes.npy`/`vels.npy`) — one shared copy across DDP ranks, no per-epoch WAV/JSON reads AND no per-step Python normalization loop; `train_renderer` auto-uses it (falls back to live loading + warns if the cache version/count is stale → **rerun precompute after adding data**), with `--workers` for prefetch. **Batch-5 refactor additions (2026-06-12, `docs/batch5-refactor.md`):** physics vectorized across algorithms (fixed D→C→B→A topo order valid for all 11 routings — no more per-algo sub-batches; torch.compile-friendly), fp64-wrapped phase, strided decimation, fp16 H2D + GPU-side cast, mean-spec reuse on resume, single-render eval, stable crc32 train/val split, pitch guard for non-pitch-conditioned runs. **Still open:** freeze-calib + precompute physics waveforms (renderer training only — can't help the matcher, whose params change every step).
